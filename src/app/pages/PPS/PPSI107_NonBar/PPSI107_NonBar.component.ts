...
export class PPSI107_NonBarComponent implements AfterViewInit {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI105Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.getPpsinptb05List();
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.updateI105Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
            const index = this.ppsinptb05List.findIndex(
              (item) => item.idx === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI105Data('2', id).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('刪除成功', ``);
  ...
  checkTemplate(worksheet, importdata) {
    if (
      ...
      worksheet.D1.v !== '機群' ||
      worksheet.E1.v !== '製程代號' ||
      worksheet.F1.v !== '尺寸MIN' ||
      worksheet.G1.v !== '尺寸MAX' ||
      worksheet.H1.v !== '生產速度'
    ) {
      this.errorMSG(
        '檔案樣板欄位表頭錯誤',
  ...
  importExcel(_data) {
    ...
    if (this.isERROR) {
      ...
      for (let i = 0; i < _data.length; i++) {
        ...
        let wkSpeed =
          _data[i].生產速度 !== undefined ? _data[i].生產速度.toString() : '0';
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.PPSService.importI105Excel('2', obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              this.loading = false;
              this.LoadingPage = false;
