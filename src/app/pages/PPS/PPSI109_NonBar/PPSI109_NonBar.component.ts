...
export class PPSI109_NonBarComponent implements AfterViewInit {
  ...
  getTbppsm012NoBarList() {
    ...
    thisForFunc.PPSService.gettbppsm012NonBarList(
      ...
      document.getElementById('li109').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a109');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '產率設定(PPSI109)';
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI109NonBarSave(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
  ...
  updateSave(rowData, _id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.upd012NonBarData(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
            const index = this.tbppsm012NonBarList.findIndex(
              (item) => item.idx === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.del012NonBarTabData(id).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('刪除成功', ``);
  ...
  importExcel(_data) {
    ...
    for (let i = 0; i < _data.length; i++) {
      ...
      if (this.importdata_new.includes(allData)) {
        this.errorTXT.push(
          `第 ` + (i + 2) + `列站別+鋼種資料重複，請檢查` + '<BR/>'
        );
        this.isERROR = true;
      } else {
        this.importdata_new.push(allData);

        if (_data[i]['站別'] == undefined) _data[i]['站別'] = '';
        if (_data[i]['鋼種'] == undefined) _data[i]['鋼種'] = '';
        if (_data[i]['產率設定值'] == undefined) _data[i]['產率設定值'] = '';
      }
      upload_data.push({
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI109NonBarExcel(obj).subscribe(
        (res) => {
  ...
  getI109ShopCodeList() {
    ...
    this.PPSService.getI109ShopCodeList(postData).subscribe(
      (res) => {
        let result: any = res;
        if (result.length > 0) {
  ...
  getI109GradeNoList() {
    ...
    this.PPSService.getI109GradeNoList(postData).subscribe(
      (res) => {
        let result: any = res;
        if (result.length > 0) {
