...
export class PPSI103_NonBarComponent implements AfterViewInit {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI102Tab1Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.getPPSINP02List();
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(obj);
      myObj.PPSService.updateI102Tab1Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI102Tab1Data('2', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.sucessMSG('刪除成功', ``);
            this.getPPSINP02List();
  ...
  importExcel(_data) {
    ...
    if (this.isERROR) {
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.PPSService.importI102Excel('2', obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              this.loading = false;
              this.LoadingPage = false;
