...
export class PPSI200Component implements AfterViewInit {
  ...
  compareWeekFn = (o1: any, o2: any) =>
    o1 && o2 ? o1.label === o2.label : o1 === o2;
  //
  ...
  weekEditFunc(id, index) {
    ...
    console.log(
      'this.selectOptionWeekDay:' + JSON.stringify(this.selectOptionWeekDay)
    );
  ...
  updateTab4Sava(_obj) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.getPPSService.updateTab4Save(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.sucessMSG('修改成功', ``);
            this.getFCPTB16List();
  ...
  getFCPTB16List() {
    ...
    this.getPPSService.getPPSFCPTB16List().subscribe((res) => {
      document.getElementById('li200').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a200');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '系統設定(PPSI200)';
      ...
      console.log(
        'init select weeekdata' + JSON.stringify(this.selectOptionWeek)
      );
  ...
  statusSaveEdit(id, index) {
    ...
    console.log(
      'switch value: ' + JSON.stringify(this.FCPRSData[index].statusFlag)
    );
    let obj = {
  ...
  updateTab5Sava(_obj) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.getPPSService.updateTab5Save(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.sucessMSG('修改成功', ``);
            this.getFCPTB16StatusList();
  ...
  delStatus(_id) {
    ...
    return new Promise((resolve, reject) => {
      let _ID = _id;
      myObj.getPPSService.delI200Tab5Data(_ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.sucessMSG('刪除成功', ``);
            this.getFCPTB16StatusList();
  ...
  insertSave(_type) {
    if (_type === 4) {
      ...
      console.log(
        'l: ' +
          this.weekDayValueForAdd.label +
          'v: ' +
          this.weekDayValueForAdd.value
      );
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.getPPSService.insertTab4Save(obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              ...
              this.errorMSG(
                '新增失敗',
                '新增的資料中，設定年月值已有相同值存在於資料庫，請檢察新增的資料'
              );
            }
          },
          (err) => {
        ...
        myObj.getPPSService.updateTab4Save(obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              ...
              const index = this.FCPTB16List.findIndex(
                (item) => item.id === _id
              );
  ...
  importExcel(_data) {
    ...
    for (let i = 0; i < _data.length; i++) {
      ...
      if (_data[i]['BALANCE_RULE'] == undefined) _data[i]['BALANCE_RULE'] = '';
      if (_data[i]['ORDER_SEQ'] == undefined) _data[i]['ORDER_SEQ'] = '';
