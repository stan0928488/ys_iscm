...
export class PPSI105Component implements AfterViewInit {
  ...
  getPPSINP04List() {
    ...
    this.PPSService.getPPSINP04List('1').subscribe((res) => {
      document.getElementById('li105').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a105');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '大調機(PPSI105)';
      ...
      for (let i = 0; i < this.PPSINP04List_tmp.length; i++) {
        data.push({
          ...
          SMALL_ADJUST_TOLERANCE_4:
            this.PPSINP04List_tmp[i].SMALL_ADJUST_TOLERANCE,
  ...
  saveEdit(id: string): void {
    let myObj = this;
    if (
      this.editCache4[id].data.EQUIP_CODE_4 === undefined ||
      '' === this.editCache4[id].data.EQUIP_CODE_4
    ) {
      myObj.message.create('error', '「機台」不可為空');
      return;
    } else if (
      this.editCache4[id].data.DIA_MIN_4 === undefined ||
      '' === this.editCache4[id].data.DIA_MIN_4.toString()
    ) {
      myObj.message.create('error', '「產出尺寸最小值」不可為空');
      return;
    } else if (
      this.editCache4[id].data.DIA_MAX_4 === undefined ||
      '' === this.editCache4[id].data.DIA_MAX_4.toString()
    ) {
      myObj.message.create('error', '「產出尺寸最大值」不可為空');
      return;
    } else if (
      this.editCache4[id].data.SHAPE_TYPE_4 === undefined ||
      '' === this.editCache4[id].data.SHAPE_TYPE_4
    ) {
      myObj.message.create('error', '「產出型態」不可為空');
      return;
    } else if (
      this.editCache4[id].data.BIG_ADJUST_CODE_4 === undefined ||
      '' === this.editCache4[id].data.BIG_ADJUST_CODE_4
    ) {
      myObj.message.create('error', '「大調機代碼」不可為空');
      return;
    } else if (
      this.editCache4[id].data.SMALL_ADJUST_TOLERANCE_4 === undefined ||
      '' === this.editCache4[id].data.SMALL_ADJUST_TOLERANCE_4
    ) {
      myObj.message.create('error', '「小調機公差標準」不可為空');
      return;
    } else if (
      this.editCache4[id].data.FURANCE_BATCH_QTY_4 === undefined ||
      '' === this.editCache4[id].data.FURANCE_BATCH_QTY_4.toString()
    ) {
      myObj.message.create('error', '「爐批數量」不可為空');
      return;
    } else {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI104Tab1Save('1', obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.extend(obj, {
        ...
        SMALL_ADJUST_TOLERANCE:
          this.editCache4[_id].data.SMALL_ADJUST_TOLERANCE_4,
        FURANCE_BATCH_QTY: this.editCache4[_id].data.FURANCE_BATCH_QTY_4,
        USERNAME: this.USERNAME,
        DATETIME: moment().format('YYYY-MM-DD HH:mm:ss'),
      });
      myObj.PPSService.updateI104Tab1Save('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.EQUIP_CODE_4 = undefined;
            this.DIA_MIN_4 = undefined;
            ...
            const index = this.PPSINP04List.findIndex(
              (item) => item.id === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI104Tab1Data('1', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.EQUIP_CODE_4 = undefined;
            this.DIA_MIN_4 = undefined;
  ...
  importExcel(_data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI104Excel('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.loading = false;
            this.LoadingPage = false;
  ...
  convertToExcel() {
    ...
    for (let i = 0; i < this.displayPPSINP04List.length; i++) {
      var ppsInp04 = {
        ...
        SMALL_ADJUST_TOLERANCE:
          this.displayPPSINP04List[i].SMALL_ADJUST_TOLERANCE_4,
