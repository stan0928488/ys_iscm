...
export class PPSI111Component implements AfterViewInit {
  ...
  getFCPTB26List() {
    ...
    this.getPPSService.getFCPTB26List().subscribe((res) => {
      document.getElementById('li111').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a111');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = 'combine資料設定(PPSI111)';
  ...
  getEQUIP_CODEList(_ShopArr) {
    if (_ShopArr.toString() !== '') {
      ...
      let myObj = this;
      this.getPPSService
        .getPickerShopEQUIP('2', _ShopArr.toString())
        .subscribe((res) => {
          ...
          for (let i = 0; i < this.EQUIP_CODEList.length; i++) {
            newres.push({
              SHOP_CODE: this.EQUIP_CODEList[i].SHOP_CODE,
              value: this.EQUIP_CODEList[i].EQUIP_CODE,
              checked: false,
            });
          }
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.getPPSService.insertTab1Save(obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.getPPSService.updateTab1Save(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.COLUMN_NAME = undefined;
            this.PickShopCode = [];
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.getPPSService.delI200Data(_ID, 1).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.COLUMN_NAME = undefined;
            this.PickShopCode = [];
