...
export class PPSI104_NonBarComponent implements AfterViewInit {
  ...
  saveEdit(id: number): void {
    ...
    if (this.editCache[id].data.SHOP_CODE === undefined) {
      ...
      this.editCache[id].data.EQUIP_CODE === undefined &&
      this.editCache[id].data.EQUIP_GROUP === undefined
    ) {
      myObj.message.create('error', '「機台」和「機群」至少填一項');
      return;
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI103Tab1Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.getPPSINP03List();
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.updateI103Tab1Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI103Tab1Data('2', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('刪除成功', ``);
  ...
  checkTemplate(worksheet, importdata) {
    if (
      ...
      worksheet.C1.v !== '機台' ||
      worksheet.D1.v !== '機群' ||
      worksheet.E1.v !== '上下料時間' ||
      worksheet.F1.v !== '搬運時間' ||
      worksheet.G1.v !== '大調機時間' ||
      worksheet.H1.v !== '小調機時間' ||
      worksheet.I1.v !== '退料時間' ||
      worksheet.J1.v !== '冷卻時間' ||
      worksheet.K1.v !== '其他整備時間'
    ) {
      this.errorMSG(
        '檔案樣板欄位表頭錯誤',
        '請先下載資料後，再透過該檔案調整上傳。'
  ...
  importExcel(_data) {
    ...
    if (this.isERROR) {
      ...
      for (let i = 0; i < _data.length; i++) {
        ...
        let loadTime =
          _data[i].上下料時間 !== undefined
            ? _data[i].上下料時間.toString()
            : '0';
        let transferTime =
          _data[i].搬運時間 !== undefined ? _data[i].搬運時間.toString() : '0';
        let bigAdjustTime =
          _data[i].大調機時間 !== undefined
            ? _data[i].大調機時間.toString()
            : '0';
        let smallAdjustTime =
          _data[i].小調機時間 !== undefined
            ? _data[i].小調機時間.toString()
            : '0';
        let returnTime =
          _data[i].退料時間 !== undefined ? _data[i].退料時間.toString() : '0';
        let coolingTime =
          _data[i].冷卻時間 !== undefined ? _data[i].冷卻時間.toString() : '0';
        let otherTime =
          _data[i].其他整備時間 !== undefined
            ? _data[i].其他整備時間.toString()
            : '0';
        ...
        this.importdata_new.push({
          ...
          bigAdjustTime: bigAdjustTime,
          smallAdjustTime: smallAdjustTime,
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.PPSService.importI103Excel('2', obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              this.loading = false;
              this.LoadingPage = false;
