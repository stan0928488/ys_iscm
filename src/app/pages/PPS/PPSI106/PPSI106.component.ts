...
export class PPSI106Component implements AfterViewInit {
  ...
  getPPSINP17List() {
    ...
    this.PPSService.getPPSINP17List().subscribe((res) => {
      document.getElementById('li106').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a106');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '小調機(PPSI106)';
      ...
      for (let i = 0; i < this.PPSINP17List_tmp.length; i++) {
        data.push({
          ...
          SMALL_ADJUST_TOLERANCE_17:
            this.PPSINP17List_tmp[i].SMALL_ADJUST_TOLERANCE,
  ...
  saveEdit(id: string): void {
    let myObj = this;
    if (
      this.editCache17[id].data.EQUIP_CODE_17 === undefined ||
      '' === this.editCache17[id].data.EQUIP_CODE_17
    ) {
      myObj.message.create('error', '「機台」不可為空');
      return;
    } else if (
      this.editCache17[id].data.DIA_MIN_17 === undefined ||
      '' === this.editCache17[id].data.DIA_MIN_17.toString()
    ) {
      myObj.message.create('error', '「產出尺寸最小值」不可為空');
      return;
    } else if (
      this.editCache17[id].data.DIA_MAX_17 === undefined ||
      '' === this.editCache17[id].data.DIA_MAX_17.toString()
    ) {
      myObj.message.create('error', '「產出尺寸最大值」不可為空');
      return;
    } else if (
      this.editCache17[id].data.SHAPE_TYPE_17 === undefined ||
      '' === this.editCache17[id].data.SHAPE_TYPE_17
    ) {
      myObj.message.create('error', '「產出型態」不可為空');
      return;
    } else if (
      this.editCache17[id].data.SMALL_ADJUST_CODE_17 === undefined ||
      '' === this.editCache17[id].data.SMALL_ADJUST_CODE_17
    ) {
      myObj.message.create('error', '「小調機代碼」不可為空');
      return;
    } else if (
      this.editCache17[id].data.SMALL_ADJUST_TOLERANCE_17 === undefined ||
      '' === this.editCache17[id].data.SMALL_ADJUST_TOLERANCE_17
    ) {
      myObj.message.create('error', '「小調機公差標準」不可為空');
      return;
    } else if (
      this.editCache17[id].data.FURANCE_BATCH_QTY_17 === undefined ||
      '' === this.editCache17[id].data.FURANCE_BATCH_QTY_17.toString()
    ) {
      myObj.message.create('error', '「爐批數量」不可為空');
      return;
    } else {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI117Tab1Save(obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.extend(obj, {
        ...
        SMALL_ADJUST_TOLERANCE:
          this.editCache17[_id].data.SMALL_ADJUST_TOLERANCE_17,
        FURANCE_BATCH_QTY: this.editCache17[_id].data.FURANCE_BATCH_QTY_17,
        USERNAME: this.USERNAME,
        DATETIME: moment().format('YYYY-MM-DD HH:mm:ss'),
      });
      myObj.PPSService.updateI117Tab1Save(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.EQUIP_CODE_17 = undefined;
            this.DIA_MIN_17 = undefined;
            ...
            this.FURANCE_BATCH_QTY_17 = undefined;
            this.sucessMSG('修改成功', ``);
            const index = this.PPSINP17List.findIndex(
              (item) => item.id === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI117Tab1Data(_ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.EQUIP_CODE_17 = undefined;
            this.DIA_MIN_17 = undefined;
  ...
  importExcel(_data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI117Excel('1', obj).subscribe(
        (res) => {
