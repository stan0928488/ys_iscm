...
@Component({
  selector: 'app-PPSI106_NonBar',
  templateUrl: './PPSI106_NonBar.component.html',
  styleUrls: ['./PPSI106_NonBar.component.scss'],
  providers: [NzMessageService],
})
export class PPSI106_NonBarComponent implements AfterViewInit {
  LoadingPage = false;
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI106Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.getTbppsm013List();
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.updateI106Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI106Data('2', id).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('刪除成功', ``);
  ...
  importExcel(_data) {
    ...
    if (this.isERROR) {
      ...
      for (let i = 0; i < _data.length; i++) {
        ...
        let groupAmount =
          _data[i].機群設備數量 !== undefined
            ...
            : '0';
        let equipQuanity =
          _data[i].單設備生產量 !== undefined
            ? _data[i].單設備生產量.toString()
            : '0';
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.PPSService.importI106Excel('2', obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              this.loading = false;
              this.LoadingPage = false;
