...
export class PPSI109Component implements AfterViewInit {
      ...
      myObj.PPSService.upd012BarData(obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            ...
            const index = this.tbppsm012List.findIndex(
              (item) => item.ID === rowData
            );
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI109Save(obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  checkTemplate(worksheet, importdata) {
    if (
      ...
      worksheet.A1.v !== '站別' ||
      worksheet.B1.v !== '機群' ||
      worksheet.C1.v !== '產率設定類型' ||
      worksheet.D1.v !== '產率設定值'
    ) {
      this.errorMSG(
        '檔案樣板欄位表頭錯誤',
        '請先下載資料後，再透過該檔案調整上傳。'
  ...
  importExcel(_data) {
    ...
    for (let i = 0; i < _data.length; i++) {
      ...
      if (this.importdata_repeat.includes(allData)) {
        ...
        if (_data[i]['機群'] == undefined) _data[i]['機群'] = '';
        if (_data[i]['設定類型'] == undefined) _data[i]['設定類型'] = '';
        if (_data[i]['設定值'] == undefined) _data[i]['設定值'] = '';
      }
      upload_data.push({
        USERNAME: this.USERNAME,
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI109Excel(obj).subscribe(
        (res) => {
  ...
  excelExport() {
    let exportData = [];
    this.PPSService.gettbppsm012List(this.pageIndex, this.pageSize).subscribe(
      (res) => {
        let result: any = res;
        ...
        for (var i = 0; i <= result.length; i++) {
          ...
          if (element) {
            var obj = {
              ...
              產率設定類型: element['YIELD_TYPE']
                ? element['YIELD_TYPE']
                : null,
              產率設定值: element['YIELD_VALUE']
                ? element['YIELD_VALUE']
                : null,
            };
            exportData.push(obj);
