...
export class PPSI104Component implements AfterViewInit {
  ...
  getPPSINP03List() {
    ...
    this.PPSService.getPPSINP03List('1').subscribe((res) => {
      document.getElementById('li104').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a104');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '整備時間(PPSI104)';
  ...
  saveEdit(id: string): void {
    let myObj = this;
    if (
      this.editCache3[id].data.SHOP_CODE === undefined ||
      '' === this.editCache3[id].data.SHOP_CODE
    ) {
      myObj.message.create('error', '「站別」不可為空');
      return;
    } else if (
      this.editCache3[id].data.EQUIP_CODE === undefined ||
      '' === this.editCache3[id].data.EQUIP_CODE
    ) {
      myObj.message.create('error', '「機台」不可為空');
      return;
    } else if (
      this.editCache3[id].data.LOAD_TIME === undefined ||
      '' === this.editCache3[id].data.LOAD_TIME.toString()
    ) {
      myObj.message.create('error', '「上下料」不可為空');
      return;
    } else if (
      this.editCache3[id].data.TRANSFER_TIME === undefined ||
      '' === this.editCache3[id].data.TRANSFER_TIME.toString()
    ) {
      myObj.message.create('error', '「搬運」不可為空');
      return;
    } else if (
      this.editCache3[id].data.OTHER_TIME === undefined ||
      '' === this.editCache3[id].data.OTHER_TIME.toString()
    ) {
      myObj.message.create('error', '「其他整備」不可為空');
      return;
    } else if (
      this.editCache3[id].data.BIG_ADJUST_TIME === undefined ||
      '' === this.editCache3[id].data.BIG_ADJUST_TIME.toString()
    ) {
      myObj.message.create('error', '「大調機」不可為空');
      return;
    } else if (
      this.editCache3[id].data.SMALL_ADJUST_TIME === undefined ||
      '' === this.editCache3[id].data.SMALL_ADJUST_TIME.toString()
    ) {
      myObj.message.create('error', '「小調機」不可為空');
      return;
    } else if (
      this.editCache3[id].data.RETURN_TIME === undefined ||
      '' === this.editCache3[id].data.RETURN_TIME.toString()
    ) {
      myObj.message.create('error', '「退料」不可為空');
      return;
    } else if (
      this.editCache3[id].data.COOLING_TIME === undefined ||
      '' === this.editCache3[id].data.COOLING_TIME.toString()
    ) {
      myObj.message.create('error', '「冷卻」不可為空');
      return;
    } else {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(obj);
      myObj.PPSService.insertI103Tab1Save('1', obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.extend(obj, {
        ...
        EQUIP_GROUP:
          this.editCache3[_id].data.EQUIP_GROUP === undefined
            ? null
            : this.editCache3[_id].data.EQUIP_GROUP,
      ...
      myObj.PPSService.updateI103Tab1Save('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE = undefined;
            this.EQUIP_CODE = undefined;
            ...
            const index = this.PPSINP03List.findIndex(
              (item) => item.id === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI103Tab1Data('1', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE = undefined;
            this.EQUIP_CODE = undefined;
  ...
  importExcel(_data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI103Excel('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.loading = false;
            this.LoadingPage = false;
