...
export class PPSI103Component implements AfterViewInit {
  ...
  getPPSINP02List() {
    ...
    this.PPSService.getPPSINP02List('1').subscribe((res) => {
      document.getElementById('li103').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a103');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '設備能力(PPSI103)';
      ...
      for (let i = 0; i < this.PPSINP02List_tmp.length; i++) {
        data.push({
          ...
          CAPABILITY_LENGTH_MIN_2:
            this.PPSINP02List_tmp[i].CAPABILITY_LENGTH_MIN,
          CAPABILITY_LENGTH_MAX_2:
            this.PPSINP02List_tmp[i].CAPABILITY_LENGTH_MAX,
  ...
  saveEdit(id: string): void {
    ...
    let myObj = this;
    if (
      this.editCache2[id].data.SHOP_CODE_2 === undefined ||
      '' === this.editCache2[id].data.SHOP_CODE_2
    ) {
      myObj.message.create('error', '「站別」不可為空');
      return;
    } else if (
      this.editCache2[id].data.EQUIP_GROUP_2 === undefined ||
      '' === this.editCache2[id].data.EQUIP_GROUP_2
    ) {
      myObj.message.create('error', '「機台群組」不可為空');
      return;
    } else if (
      this.editCache2[id].data.EQUIP_CODE_2 === undefined ||
      '' === this.editCache2[id].data.EQUIP_CODE_2
    ) {
      myObj.message.create('error', '「機台」不可為空');
      return;
    } else if (
      this.editCache2[id].data.PROCESS_CODE_2 === undefined ||
      '' === this.editCache2[id].data.PROCESS_CODE_2
    ) {
      myObj.message.create('error', '「製程碼」不可為空');
      return;
    } else if (
      this.editCache2[id].data.GRADE_GROUP_2 === undefined ||
      '' === this.editCache2[id].data.GRADE_GROUP_2
    ) {
      myObj.message.create('error', '「鋼種類別」不可為空');
      return;
    } else if (
      this.editCache2[id].data.SHAPE_TYPE_2 === undefined ||
      '' === this.editCache2[id].data.SHAPE_TYPE_2
    ) {
      myObj.message.create('error', '「形狀」不可為空');
      return;
    } else if (
      this.editCache2[id].data.INPUT_DIA_MAX_2 === undefined ||
      '' === this.editCache2[id].data.INPUT_DIA_MAX_2.toString()
    ) {
      myObj.message.create('error', '「投入尺寸上限」不可為空');
      return;
    } else if (
      this.editCache2[id].data.CAPABILITY_DIA_MIN_2 === undefined ||
      '' === this.editCache2[id].data.CAPABILITY_DIA_MIN_2.toString()
    ) {
      myObj.message.create('error', '「設備能力最小尺寸」不可為空');
      return;
    } else if (
      this.editCache2[id].data.CAPABILITY_DIA_MAX_2 === undefined ||
      '' === this.editCache2[id].data.CAPABILITY_DIA_MAX_2.toString()
    ) {
      myObj.message.create('error', '「設備能力最大尺寸」不可為空');
      return;
    } else if (
      this.editCache2[id].data.CAPABILITY_LENGTH_MIN_2 === undefined ||
      '' === this.editCache2[id].data.CAPABILITY_LENGTH_MIN_2.toString()
    ) {
      myObj.message.create('error', '「設備能力最小長度」不可為空');
      return;
    } else if (
      this.editCache2[id].data.CAPABILITY_LENGTH_MAX_2 === undefined ||
      '' === this.editCache2[id].data.CAPABILITY_LENGTH_MAX_2.toString()
    ) {
      myObj.message.create('error', '「設備能力最大長度」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTIMAL_DIA_MIN_2 === undefined ||
      '' === this.editCache2[id].data.OPTIMAL_DIA_MIN_2.toString()
    ) {
      myObj.message.create('error', '「最佳能力最小尺寸」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTIMAL_DIA_MAX_2 === undefined ||
      '' === this.editCache2[id].data.OPTIMAL_DIA_MAX_2.toString()
    ) {
      myObj.message.create('error', '「最佳能力最大尺寸」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTIMAL_LENGTH_MIN_2 === undefined ||
      '' === this.editCache2[id].data.OPTIMAL_LENGTH_MIN_2.toString()
    ) {
      myObj.message.create('error', '「最佳能力最小長度」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTIMAL_LENGTH_MAX_2 === undefined ||
      '' === this.editCache2[id].data.OPTIMAL_LENGTH_MAX_2.toString()
    ) {
      myObj.message.create('error', '「上最佳能力最大長度」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTION_EQUIP_1_2 === undefined ||
      '' === this.editCache2[id].data.OPTION_EQUIP_1_2.toString()
    ) {
      myObj.message.create('error', '「替代機台順位1」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTION_EQUIP_2_2 === undefined ||
      '' === this.editCache2[id].data.OPTION_EQUIP_2_2.toString()
    ) {
      myObj.message.create('error', '「替代機台順位2」不可為空');
      return;
    } else if (
      this.editCache2[id].data.OPTION_EQUIP_3_2 === undefined ||
      '' === this.editCache2[id].data.OPTION_EQUIP_3_2.toString()
    ) {
      myObj.message.create('error', '「替代機台順位3」不可為空');
      return;
    } else {
  ...
  insertSave(_type) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI102Tab1Save('1', obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.extend(obj, {
        ...
        CAPABILITY_LENGTH_MIN:
          this.editCache2[_id].data.CAPABILITY_LENGTH_MIN_2,
        CAPABILITY_LENGTH_MAX:
          this.editCache2[_id].data.CAPABILITY_LENGTH_MAX_2,
      ...
      console.log(obj);
      myObj.PPSService.updateI102Tab1Save('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE_2 = undefined;
            this.EQUIP_GROUP_2 = undefined;
            ...
            const index = this.PPSINP02List.findIndex(
              (item) => item.id === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI102Tab1Data('1', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE_2 = undefined;
            this.EQUIP_GROUP_2 = undefined;
  ...
  importExcel(_data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI102Excel('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.loading = false;
            this.LoadingPage = false;
  ...
  convertToExcel() {
    ...
    for (let i = 0; i < this.displayPPSINP02List.length; i++) {
      var ppsInp02 = {
        ...
        CAPABILITY_LENGTH_MIN:
          this.displayPPSINP02List[i].CAPABILITY_LENGTH_MIN_2,
        CAPABILITY_LENGTH_MAX:
          this.displayPPSINP02List[i].CAPABILITY_LENGTH_MAX_2,
