  ...
  jsonExcelData: any[] = [];
   ...
   barchInsertExcelData(){
    ...
    return new Promise(function(resolve, reject){
      myThis.PPSService.batchSaveI113Data(myThis.jsonExcelData).subscribe(response => {
   ...
   deleteAllData(){
    const myThis = this;
    return new Promise(function(resolve, reject){
      myThis.PPSService.deleteI113AllData().subscribe(response => {
        if (response.success === true) {
          resolve("刪除所有資料成功");
        } 
        else {
   ...
   convertJsonToEnglishKey() : void {
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"鋼種類別":').join('"GRADE_NO_13":'));
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"產出型態":').join('"OUT_TYPE":'));
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"產出成品尺寸最小值":').join('"DIA_MIN":'));
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"產出成品尺寸最大值":').join('"DIA_MAX":'));
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"研磨道次":').join('"GRINDING_PASS":'));
    this.jsonExcelData = JSON.parse(JSON.stringify(this.jsonExcelData).split('"每刀研磨尺寸":').join('"GRINDING_SIZE":'));
   }
   ...
   checkAllValuesNotEmpty(jsonExcelData) : boolean{
    ...
    for (let i = 1; i <= jsonExcelData.length; i++){
      ...
      if(_.isEmpty(String(jsonExcelData[i-1]["鋼種類別"])) || _.isEqual(String(jsonExcelData[i-1]["鋼種類別"]), '-')){
        this.errorMSG("匯入失敗", `第${rowNumberInExcel}行資料的「鋼種類別」不得為空，請修正`);
        return false;
      }
      ...
      if(_.isEmpty(String(jsonExcelData[i-1]["產出成品尺寸最小值"])) || _.isEqual(String(jsonExcelData[i-1]["產出成品尺寸最小值"]), '-')){
        this.errorMSG("匯入失敗", `第${rowNumberInExcel}行資料的「產出成品尺寸最小值」不得為空，請修正`);
        return false;
      }

      if(_.isEmpty(String(jsonExcelData[i-1]["產出成品尺寸最大值"])) || _.isEqual(String(jsonExcelData[i-1]["產出成品尺寸最大值"]), '-')){
        this.errorMSG("匯入失敗", `第${rowNumberInExcel}行資料的「產出成品尺寸最大值」不得為空，請修正`);
        return false;
      }

      if(_.isEmpty(String(jsonExcelData[i-1]["研磨道次"])) || _.isEqual(String(jsonExcelData[i-1]["研磨道次"]), '-')){
        this.errorMSG("匯入失敗", `第${rowNumberInExcel}行資料的「研磨道次」不得為空，請修正`);
        return false;
      }

      if(_.isEmpty(String(jsonExcelData[i-1]["每刀研磨尺寸"])) || _.isEqual(String(jsonExcelData[i-1]["每刀研磨尺寸"]), '-')){
        this.errorMSG("匯入失敗", `第${rowNumberInExcel}行資料的「每刀研磨尺寸」不得為空，請修正`);
        return false;
      }
    } //end for
   ...
   exportToExcel() : void {
    ...
    const firstRow = ["GRADE_NO_13", "OUT_TYPE", "DIA_MIN", "DIA_MAX", "GRINDING_PASS", "GRINDING_SIZE"];
    const firstRowDisplay = {GRADE_NO_13:"鋼種類別", OUT_TYPE:"產出型態", DIA_MIN:"產出成品尺寸最小值", DIA_MAX:"產出成品尺寸最大值", GRINDING_PASS:"研磨道次", GRINDING_SIZE:"每刀研磨尺寸"};
