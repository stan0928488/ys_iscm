...
export class PPSI108_NonBarComponent implements AfterViewInit {
  ...
  getPpsinptb08List() {
    ...
    this.PPSService.getPPSINP08List('2').subscribe((res) => {
      document.getElementById('li108').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a108');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '非線速工時(PPSI108)';
  ...
  saveEdit(id: number): void {
    ...
    if (this.editCache[id].data.schShopCode === undefined) {
      ...
      this.editCache[id].data.equipCode === undefined &&
      this.editCache[id].data.equipGroup === undefined
    ) {
      myObj.message.create('error', '「機台」和「機群」至少填一項');
      return;
    } else {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI108Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.getPpsinptb08List();
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.updateI108Save('2', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('修改成功', ``);
            const index = this.ppsinptb08List.findIndex(
              (item) => item.idx === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI108Data('2', id).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.onInit();
            this.sucessMSG('刪除成功', ``);
  ...
  checkTemplate(worksheet, importdata) {
    if (
      ...
      worksheet.F1.v !== '鋼種' ||
      worksheet.G1.v !== '包裝碼' ||
      worksheet.H1.v !== '尺寸MIN' ||
      worksheet.I1.v !== '尺寸MAX' ||
      worksheet.J1.v !== '加工時間'
    ) {
      this.errorMSG(
        '檔案樣板欄位表頭錯誤',
  ...
  importExcel(_data) {
    ...
    if (this.isERROR) {
      ...
      for (let i = 0; i < _data.length; i++) {
        ...
        let equipProcessCode =
          _data[i].製程代號 !== undefined ? _data[i].製程代號.toString() : '';
        let steelType =
          _data[i].鋼種 !== undefined ? _data[i].鋼種.toString() : '';
        let packCode =
          _data[i].包裝碼 !== undefined ? _data[i].包裝碼.toString() : '';
        let diaMin =
          _data[i].尺寸MIN !== undefined ? _data[i].尺寸MIN.toString() : '0';
        let diaMax =
          _data[i].尺寸MAX !== undefined ? _data[i].尺寸MAX.toString() : '0';
        let wkTime =
          _data[i].加工時間 !== undefined ? _data[i].加工時間.toString() : '0';
        this.importdata_new.push({
          plantCode: plantCode,
          schShopCode: schShopCode,
          equipCode: equipCode,
          equipGroup: equipGroup,
          equipProcessCode: equipProcessCode,
          steelType: steelType,
          packCode: packCode,
          diaMin: diaMin,
          diaMax: diaMax,
      ...
      return new Promise((resolve, reject) => {
        ...
        myObj.PPSService.importI108Excel('2', obj).subscribe(
          (res) => {
            if (res[0].MSG === 'Y') {
              this.loading = false;
              this.LoadingPage = false;
