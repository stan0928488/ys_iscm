...
export class PPSI107Component implements AfterViewInit {
  ...
  getPPSINP05List() {
    ...
    this.PPSService.getPPSINP05List('1').subscribe((res) => {
      document.getElementById('li107').style.backgroundColor = '#E4E3E3';
      const a = document.getElementById('a107');
      a.style.cssText = 'color: blue; font-weight:bold;';
      a.innerHTML = '線速工時(PPSI107)';
  ...
  saveEdit(id: string): void {
    ...
    let myObj = this;
    if (
      this.editCache05[id].data.SHOP_CODE_5 === undefined ||
      '' === this.editCache05[id].data.SHOP_CODE_5
    ) {
      myObj.message.create('error', '「站號」不可為空');
      return;
    } else if (
      this.editCache05[id].data.EQUIP_CODE_5 === undefined ||
      '' === this.editCache05[id].data.EQUIP_CODE_5
    ) {
      myObj.message.create('error', '「機台」不可為空');
      return;
    } else if (
      this.editCache05[id].data.SHAPE_TYPE_5 === undefined ||
      '' === this.editCache05[id].data.SHAPE_TYPE_5
    ) {
      myObj.message.create('error', '「產出型態」不可為空');
      return;
    } else if (
      this.editCache05[id].data.GRADE_GROUP_5 === undefined ||
      '' === this.editCache05[id].data.GRADE_GROUP_5
    ) {
      myObj.message.create('error', '「鋼種類別」不可為空');
      return;
    } else if (
      this.editCache05[id].data.REDUCTION_RATE_MIN_5 === undefined ||
      '' === this.editCache05[id].data.REDUCTION_RATE_MIN_5.toString()
    ) {
      myObj.message.create('error', '「減面率MIN」不可為空');
      return;
    } else if (
      this.editCache05[id].data.REDUCTION_RATE_MAX_5 === undefined ||
      '' === this.editCache05[id].data.REDUCTION_RATE_MAX_5.toString()
    ) {
      myObj.message.create('error', '「減面率MAX」不可為空');
      return;
    } else if (
      this.editCache05[id].data.DIA_MIN_5 === undefined ||
      '' === this.editCache05[id].data.DIA_MIN_5.toString()
    ) {
      myObj.message.create('error', '「最小產出尺寸不可為空');
      return;
    } else if (
      this.editCache05[id].data.DIA_MAX_5 === undefined ||
      '' === this.editCache05[id].data.DIA_MAX_5.toString()
    ) {
      myObj.message.create('error', '「最大產出尺寸」不可為空');
      return;
    } else if (
      this.editCache05[id].data.SPEED_5 === undefined ||
      '' === this.editCache05[id].data.SPEED_5.toString()
    ) {
      myObj.message.create('error', '「線速」不可為空');
      return;
    } else if (
      this.editCache05[id].data.EQUIP_CAP_5 === undefined ||
      '' === this.editCache05[id].data.EQUIP_CAP_5.toString()
    ) {
      myObj.message.create('error', '「日產出量」不可為空');
      return;
    } else {
  ...
  insertSave() {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.insertI105Save('1', obj).subscribe(
        (res) => {
          console.log(res);
          if (res[0].MSG === 'Y') {
  ...
  updateSave(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.updateI105Save('1', obj).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE_5 = undefined;
            this.EQUIP_CODE_5 = undefined;
            ...
            this.getPPSINP05List();
            this.sucessMSG('修改成功', ``);
            const index = this.PPSINP05List.findIndex(
              (item) => item.id === _id
            );
  ...
  delID(_id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.delI105Data('1', _ID).subscribe(
        (res) => {
          if (res[0].MSG === 'Y') {
            this.SHOP_CODE_5 = undefined;
            this.EQUIP_CODE_5 = undefined;
  ...
  importExcel(_data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      myObj.PPSService.importI105Excel('1', obj).subscribe(
        (res) => {
