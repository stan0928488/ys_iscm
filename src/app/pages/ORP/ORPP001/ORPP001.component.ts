...
export class ORPP001Component implements AfterViewInit {
  ...
  columnDefs : ColDef[]   = [
    { headerName: '客戶鋼種', field: 'gradeNoCust', width: 150, headerComponent : AGCustomHeaderComponent, headerComponentParams:this.agCustomHeaderParams},
        ...
        upload$.subscribe({
          next: (res) => {
            ...
            myArray.forEach(item => {
              ...
              delete item.saleOrderWeight; // 删除原始的属性
            });
            for(var i = 0; i < myArray.length; i++) {
              var list = myArray[i];
              console.log(list);
              
              this.addDataList.push(list);
            }
            this.status = 'success';
          },
          error: (error: any) => {
